...
async function fetchRoute(points) {
  const path = points.map((p) => `${p.lng},${p.lat}`).join(";") + "?overview=full&geometries=geojson";
...
async function planRoute() {
  ...
  els.sheet.hidden = true;
  els.btnPlan.hidden = true;
  hint("Planning the walk", "First stop to last stop, door to door.");
  hideSketch();
...
async function init() {
  ...
  els.btnYes.addEventListener("click", () => {
    const p = pending()[0]; if (!p) return; p.verdict = "yes"; openCard();
  });
  els.btnNo.addEventListener("click", () => {
    const p = pending()[0]; if (!p) return; p.verdict = "no"; rejected.push(p.id); openCard();
  });
  els.btnUndo.addEventListener("click", () => {
    const id = rejected.pop();
    const p = candidates.find((c) => c.id === id);
    if (p) p.verdict = "pending";
    openCard();
  });
}
